...
func grow_mario() -> void:
	# 1. Flip Mario's internal state structural tracking variable to Super!
	current_form = Form.SUPER
	
	# 2. Immediately trigger his collision layer matrix update 
	update_collision_states()
	
	# 3. Force the animation engine to instantly update to the super_ frames
	update_animation_engine(0.0, false)
	
	print(">>> GDScript: Mario has successfully grown into SUPER form! <<<")
